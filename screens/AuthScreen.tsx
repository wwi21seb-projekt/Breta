...
const AuthScreen = () => {
  ...
  return (
    <View style={{flex: 1, marginTop: 60}}>
      <View style={{flexDirection: "row", marginVertical: 40}}>
        ...
        >
          <Text style={{ color: COLORS.black, fontSize: SIZES.large }}>Registrierung</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            styles.buttonSwitch,
  ...
  onBlur={()=> {if (!emailRegex.test(email)) {
  ...
  {errorEmail && <Text style={styles.error}>{errorEmail}</Text>}
          ...
          <TouchableOpacity style={[styles.loginButton, { backgroundColor: isEmailFilled && emailRegex.test(email) && password.length >= 8 ? COLORS.primary : COLORS.lightgray }]}
  onPress={handleLogin}
  disabled={!isEmailFilled || !emailRegex.test(email) || password.length < 8}
  >
            <Text style={{ color: COLORS.black, fontSize: SIZES.large }}>Einloggen</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
