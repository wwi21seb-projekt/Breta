    ...
    const checkUsername = () => {
        ...
        if (/^[a-zA-Z0-9]+$/.test(username)) {
          ...
          if (username.length <= 20) {
            ...
            setUsernameError('Der Nutzername darf nicht länger als 20 Zeichen sein.');
            return false;
          }
        } else {
          // Der Wert enthält Sonderzeichen oder Emojis
          setUsernameError('Der Nutzername darf keine Sonderzeichen oder Emojis enthalten.')
          return false;
        }
      };
      ...
      const checkEmail = () => {
        ...
        if (email.length <= 128) {
          ...
          // Der Wert ist zu lang
          setEmailError('Die E-Mail-Adresse darf nicht länger als 128 Zeichen sein.')
          return false;
        }
      }
      ...
      const checkPassword = () => {
        ...
        if (password.length >= 8) {
          ...
          if (/[A-Z]/.test(password) && /[a-z]/.test(password) && /\d/.test(password)) {
                ...
                setPasswordError('Das Passwort muss mindestens einen Kleinbuchtsaben, einen Großbuchstaben und eine Zahl enthalten.');
                return false;
              }
        } else {
      ...
      const checkInputFields = () => {
        ...
        let alltrue = true;
        let methods = [checkEmail(), checkUsername(), checkNickname(), checkPassword(), checkRepeatPassword()]
...
return(
<View>
          <FloatingTextInput label='Email Adresse' value={email} onChangeText={(text) => setEmail(text) 
          }/>
          {!!emailError && (
            <Text style={{color: COLORS.red, paddingBottom: 20, textAlign: 'center'}}>
              {emailError}
                </Text>)}
          <FloatingTextInput label='Nutzername' value={username} onChangeText={(text) => setUsername(text)}/>
          {!!usernameError && (
            <Text style={{color: COLORS.red, paddingBottom: 20, textAlign: 'center'}}>
              {usernameError}
                </Text>)}
          <FloatingTextInput label='Nickname(optional)' value={nickname} onChangeText={(text) => setNickname(text)}/>
            {!!nicknameError && 
              (<Text style={{color: COLORS.red, paddingBottom: 20, textAlign: 'center'}}>
              {nicknameError} 
                </Text>)}
          ...
          <FloatingTextInput secureTextEntry={true} label='Passwort' value={password} onChangeText={(text) => setPassword(text) }/>
          {!!passwordError && 
              (<Text style={{color: COLORS.red, paddingBottom: 20, textAlign: 'center'}}>
              {passwordError} 
                </Text>)}
          <FloatingTextInput secureTextEntry={true} label='Passwort bestätigen' value={repeatPassword} onChangeText={(text) => setRepeatPassword(text)}/>
          {!!repeatPasswordError && 
              (<Text style={{color: COLORS.red, paddingBottom: 20, textAlign: 'center', }}>
          ...
          <TouchableOpacity
            ...
            onPress={() => register()}
          >
            <Text style={{ color: COLORS.black, fontSize: SIZES.large }}>Registrieren</Text>
          </TouchableOpacity>
