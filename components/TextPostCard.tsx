      ...
      <View className="w-full bg-white rounded-full p-4 shadow-lg z-20 relative">
        <View className="flex-row items-center justify-between">
          ...
          <TouchableOpacity onPress={openCommentModal}>
            <Ionicons name="chatbox-ellipses-outline" size={24} className="mr-1" />
          </TouchableOpacity>
          <TouchableOpacity className="flex-row items-center" onPress={handleLikePress}>
  ...
  <View className="flex-1 justify-end bg-opacity-50">
    <View className="bg-white rounded-t-xl p-5 h-3/4">
      ...
      <ScrollView className="pt-4">
        {comments.length === 0 ? (
          <Text className="text-center text-sm py-5">Es sind noch keine Kommentare da!</Text>
        ) : (
          comments.map((comment) => (
            <View key={comment.id} className="flex-row items-start py-3 border-b border-gray">
              <Image source={{ uri: comment.profilePic }} className="w-8 h-8 rounded-full mr-3" />
