...
export const handleSubscription = async (
    ...
    const handleErrorResponse = (status: number) => {
      const errorMessages: { [key: number]: string } = {
        ...
        404: `Der Nutzer, den du ${isFollowed ? 'deabbonieren' : 'abbonieren'} möchtest, wurde nicht gefunden. Versuche es später erneut.`,
        406: "Du kannst dir nicht selbst folgen.",
        409: "Du folgst diesem Nutzer bereits."
      };
  
      setError(errorMessages[status] || "Etwas ist schiefgelaufen. Versuche es später erneut.");
    };
    ...
    const url = isFollowed ? `${baseUrl}subscriptions/${subscriptionId}` : `${baseUrl}subscriptions`;
